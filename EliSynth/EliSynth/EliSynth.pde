...
void setup() {
  ...
  knob1Last = analogRead(KNOB1);
  knob2Last = analogRead(KNOB2);
}
...
void loop () {
  ...
  if (tempNote.id != NULL)
  {
    ...
    while (tempNote.time < currentTime)
    {
      ...
      tempNote.id = NULL;
      noteQueue[tempNote.next].prev = NULL;
...
void addNoteToQueue(scaleNotes n, int v, long t, char type)
{
  ...
  int i = 0;
  while ((noteQueue[i].inUse == true) && (i < NOTE_QUEUE_LENGTH))
  {
    i++;
  }
  ...
  if (firstNote != NULL)
  {
    ...
    while ((tempNote.time < noteQueue[i].time) && (tempNote.next != NULL))
    {
      tempNote = noteQueue[tempNote.next];
    }
